/*Largest item association: Given pairs of item association, one needs to find the largest item association. If we have ties, then the association that has lower items in terms of lexicographical order will be selected. For instance, if item5 -> item6 -> item7 & item5 -> item6 -> item8 are a tie, then the former one will be chosen*/
...
/*
Time complexity: Forming adjacency list: O(no of pairs) + DFS: every item as a start, and every item will traverse the whole graph in the worst case, so O(no of items OR no of pairs) * no of pairs * string comparisions
Space complexity: HashMap for adjacency list: O(no of pairs) + visitedItem Set: O(no of items OR no of pairs) + temp result & result: O(no of pairs) + stack: O(no of items OR no of pairs) => O(no of pairs)
*/
